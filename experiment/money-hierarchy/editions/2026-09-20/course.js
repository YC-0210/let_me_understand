(()=>{
 ...
 const watching={'repay-iou':'Watch the café receive cash. Then the IOU is paid.',interbank:'Watch the banks settle; compare both customers’ balances.',cash:'Watch your $5 move to the café.',iou:'Watch your promise reach the café owner.',deposit:'Watch the bank move $5 between the two accounts.',reserves:'Watch reserves move from Bank A to Bank B.',withdraw:'Watch the bank exchange the balance for a note.',dealer:'The bond goes to the dealer; $9 goes to you.',support:'Watch reserves reach the bank. They come with a loan.'};
 ...
 function load(){const p=page();active=false;answered=false;elapsed=0;playing=false;last=0;$('title').textContent=p.title;$('speech').textContent=p.say;$('topic').textContent=labels[p.topic];$('position').textContent=`${index+1} / ${MoneyCourse.length}`;$('course-progress').max=MoneyCourse.length;$('course-progress').value=index+1;$('back').disabled=index===0;$('detail').textContent=p.more;$('reading').href='sources.html#'+(p.source||p.topic);$('more').open=false;$('feedback').textContent='';$('choices').replaceChildren();$('replay').hidden=true;$('continue').hidden=!!p.choices;$('continue').textContent=p.action||'Continue';
  (p.choices||[]).forEach((choice,i)=>{const b=document.createElement('button');b.textContent=choice;b.setAttribute('aria-pressed','false');b.onclick=()=>{if(answered)return;$('choices').querySelectorAll('button').forEach(x=>x.setAttribute('aria-pressed','false'));b.setAttribute('aria-pressed','true');if(i!==p.correct){$('speech').textContent=p.hint;$('feedback').textContent='Think about what must happen next.';paint();return;}answered=true;$('feedback').textContent='';$('speech').textContent=p.result;active=true;elapsed=6000;$('choices').hidden=true;$('continue').hidden=false;$('continue').textContent=index===MoneyCourse.length-1?'Start again':'Continue';paint();};$('choices').appendChild(b);});$('choices').hidden=false;paint();}
