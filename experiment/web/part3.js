...
window.Part3 = (() => {
  ...
  let index=0,progress=0,playing=false,choice=null,checked=false,arrival=.12,frame=0,previous=0,lastDraw=-1,done=false,winkOpen=false;
  ...
  function updateWink(){
    ...
    if(observation){const text=observation.textContent;$('#wink-current').textContent=text;if(progress>0&&cue.textContent!==text)cue.textContent=text;else if(progress===0)cue.textContent=current().wink_cue;observation.remove();}
  ...
  function go(n){stop();index=n;winkOpen=false;progress=0;choice=null;checked=false;arrival=.12;done=false;try{localStorage.setItem('part3-full-step',String(index));}catch{}mount();$('.lesson-shell').scrollIntoView({block:'start',behavior:'instant'});}
