...
window.Part3 = (() => {
  ...
  let index=0,progress=0,playing=false,choice=null,checked=false,arrival=.12,frame=0,previous=0,lastDraw=-1,done=false,winkOpen=false,guideClock=0,guideResize,motionBlend=1,overviewOpen=null,lastOverviewKey="";
  ...
  function go(n){stop();index=n;overviewOpen=null;winkOpen=false;guideClock=0;motionBlend=1;progress=0;choice=null;checked=false;arrival=.12;done=false;try{localStorage.setItem('part3-full-step',String(index));}catch{}mount();$('.lesson-shell').scrollIntoView({block:'start',behavior:'instant'});}
  ...
  function draw(){
    ...
    lastBlend=motionBlend;updateOverview();updateWink();
  ...
  function updateOverview(){
    const map=$('#sequence-map');if(!map)return;
    $('.lesson-content').classList.toggle('overview-open',overviewOpen);
    const n=timeline()?0:Math.min(current().frames.length-1,Math.floor(progress*(current().frames.length-1)+1e-7));
    const state=window.CourseOverview.state(current().chapter,current().scene,n,progress);
    const key=[current().chapter,state.active,state.warning,state.detail,overviewOpen].join('|');
    if(key!==lastOverviewKey){
      map.innerHTML=window.CourseOverview.render(current().chapter,current().scene,n,progress,overviewOpen);
      $('#sequence-toggle').onclick=()=>{stop();overviewOpen=!overviewOpen;lastOverviewKey='';draw();$('#sequence-toggle').focus();};
      lastOverviewKey=key;
    }
  }
