...
window.IntuitionGuide = (() => {
  ...
  function icon(kind){return `<svg viewBox="-48 -40 96 80" aria-hidden="true"><g fill="none" stroke="${colors[kind]||colors.socket}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">${glyphs[kind]}</g></svg>`;}
  ...
  const positions=narrow=>narrow?{browser:[60,110],browserB:[60,370],socket:[200,110],listener:[200,370],parent:[345,110],child:[345,280],record:[275,485],record2:[340,485],record3:[405,485],notice:[90,485],app:[345,113],message:[60,110]}:{browser:[65,140],browserB:[65,340],socket:[260,140],listener:[260,340],parent:[460,140],child:[690,140],record:[670,340],record2:[748,340],record3:[826,340],notice:[460,340],app:[460,145],message:[65,140]};
  function entity(id,kind,label){return `<g id="ig-${id}" data-kind="${kind}" class="ig-entity" style="opacity:0"><g fill="#16171e" stroke="${colors[kind]||colors.socket}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">${glyphs[kind]}</g><text y="58">${label}</text></g>`;}
  function canvas(){return `<div class="ig-canvas"><svg id="ig-svg" class="ig-priming" role="img" aria-labelledby="ig-svg-title ig-svg-desc"><title id="ig-svg-title"></title><desc id="ig-svg-desc"></desc><g id="ig-links"></g>${entity('browser','browser','Browser A')}${entity('browserB','browser','Browser B')}${entity('socket','socket','A’s socket')}${entity('listener','listener','Listening socket')}${entity('parent','process','Server / parent')}${entity('child','process','Child A')}${entity('app','app','')}${entity('record','record','Exit A')}${entity('record2','record','Exit B')}${entity('record3','record','Exit C')}${entity('notice','notice','SIGCHLD')}${entity('message','message','')}<g id="ig-handle-parent" class="ig-entity" data-kind="handle"></g><g id="ig-handle-child" class="ig-entity" data-kind="handle"></g><text id="ig-result" class="ig-small"></text><rect id="ig-focus" class="ig-focus-outline" width="104" height="100" rx="16"/></svg><button class="ig-wink" id="ig-wink" aria-label="Wink: explain this moment" aria-expanded="false"><svg viewBox="0 0 100 100" aria-hidden="true"><circle fill="#f5f0ff" cx="50" cy="50" r="46"/><ellipse fill="#4b3869" cx="38" cy="45" rx="6" ry="9"/><path d="M59 43Q67 55 75 43" stroke="#4b3869" stroke-width="4" fill="none"/><path fill="#b491f5" d="M76 10Q77 15 82 17Q77 19 76 24Q75 19 70 17Q75 15 76 10Z"/></svg></button></div>`;}
  ...
  function animateMessage(){
    ...
    const from=reverse?[p.socket[0]+25,p.socket[1]]:[p.browser[0]+32,p.browser[1]],to=reverse?p.browser:[p.parent[0]-60,p.parent[1]];
    const el=$('#ig-message');el.style.transform=`translate(${to[0]}px,${to[1]}px)`;
    el.animate([{transform:`translate(${from[0]}px,${from[1]}px)`},{transform:`translate(${to[0]}px,${to[1]}px)`}],{duration:matchMedia('(prefers-reduced-motion: reduce)').matches?0:1100,easing:'ease-in-out'});
  }
  function draw(){
    ...
    if(!m.child)pos.child=[...pos.parent];
    if(m.reply)pos.message=[...pos.browser];else pos.message=m.mode==='application'?[pos.parent[0]-58,pos.parent[1]]:[pos.parent[0]-60,pos.parent[1]];
    ...
    handles.forEach(([owner,visible])=>{
      const start=[pos[owner][0]-20,pos[owner][1]+35],end=[pos.socket[0]+10,pos.socket[1]+25],p=[start[0]*.65+end[0]*.35,start[1]*.65+end[1]*.35+35];
      const el=$('#ig-handle-'+owner);el.style.transform=`translate(${p[0]}px,${p[1]}px)`;el.style.opacity=visible?1:0;el.innerHTML=`<g fill="#16171e" stroke="${colors.handle}" stroke-width="2.5">${glyphs.handle}</g>`;
    ...
    if(!narrow){
      ...
      const obstacles=Object.entries(pos).filter(([id])=>show[id]&&id!=='message').map(([,p])=>({x:(p[0]-57)*scale,y:(p[1]-48)*scale,w:114*scale,h:123*scale}));
      accessPoints.forEach(p=>obstacles.push({x:(p[0]-30)*scale,y:(p[1]-15)*scale,w:60*scale,h:30*scale}));
      ...
      speech.style.width=sw+'px';speech.style.left=(best?.x||8)+'px';speech.style.top=(best?.y||svg.clientHeight-sh)+'px';
    ...
    if(m.collect){
      ...
      for(let i=0;i<m.recordCount;i++){const id=['record','record2','record3'][i],el=$('#ig-'+id);el.getAnimations().forEach(a=>a.cancel());el.animate([{transform:`translate(${pos[id][0]}px,${pos[id][1]}px)`,opacity:1},{transform:`translate(${pos.parent[0]}px,${pos.parent[1]}px) scale(.3)`,opacity:0}],{duration:reduce?0:650,delay:reduce?0:900+i*700,fill:'forwards',easing:'ease-in-out'});}
    }
  }
  return {mount,stop,model,icon};
})();
